...
선형 HTML 세그먼트 스캐너, Shadow DOM 격리, 적응형 목차(TOC) & 스크롤스파이, 정밀 텍스트 노드 매핑 캐럿 네비게이션, 사용자 주도 양방향 차등 스크롤 동기화, 원클릭 싱크 재정렬, 마크다운 표-XML 태그 렉서 경계 가드, 옵시디언 티타늄 모노크롬 테마를 지원하는 단일 파일 마크다운 에디터
...
- **옵시디언 티타늄 모노크롬 테마 (Obsidian Titanium & Platinum Monochrome)**: 글쓰기 및 텍스트 본연의 가독성에 집중할 수 있도록 유채색 틴트를 완전 제거한 초정밀 흑백 하이엔드 테마
- **실제 RP 캐릭터 시뮬레이션 프롬프트 & 통합 가이드 기본 탑재**: 실제 1:1 RP 캐릭터 엔진(`<rp_engine>`, `<character>`, `<world>`, `<state_schema>`, `<state_logic>`), 글래스모피즘 상태 모니터 HTML 위젯, 단축키 및 주요 기능 가이드
...
- **PWA 및 고해상도 앱 아이콘 / 파비콘 지원**: 브라우저 탭 파비콘(SVG/PNG/ICO), 홈 화면 바로가기(Web Manifest), Apple Touch Icon 완비
